...
class Sample:
    def __init__(self, id: int, name="sample", index=(0, 0)):
        self.id = id
        self.name = name
        self.index = index  # This is the index of the sample on the sample holder. (1,2) for example, python numeraction convention.
    ...
    def __str__(self):
        return f"Sample {self.id}: {self.name}, index: {self.index}, position: {self.position_original}"
